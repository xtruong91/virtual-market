...
namespace VirtualMarket.Common.Vault
{
    public class VaultStore : IVaultStore
    {
        ...
        public async Task<IDictionary<string, object>> GetAsync(string key)
        {
            ...
            {
                throw new VaultException("Vault secrete key cannot be empty");
            }
            try
            {
        ...
        private IAuthMethodInfo GetAuthMethod()
        {
            ...
            throw new VaultAuthTypeNotSupportedException($"Vault auth type: '{_options.AuthType}' is not support.",
                _options.AuthType);
        }
